export default async function handler(req, res) {
  const { email, includePast } = req.query;
  ...
  try {
    ...
    // Se includePast=true, prendi da 2 anni fa — altrimenti solo futuri
    const timeMin = includePast === 'true'
      ? new Date(Date.now() - 2 * 365 * 24 * 60 * 60 * 1000).toISOString()
      : new Date().toISOString();

    const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?timeMin=${timeMin}&singleEvents=true&orderBy=startTime&maxResults=200`;
