...
export default function App() {
      ...
      if (u && u.email !== ADMIN_EMAIL) {
        try {
          const [snapU, snapO] = await Promise.all([
            getDoc(doc(db, "studenti", u.uid)),
            getDocs(collection(db, "offerte")),
          ]);
          setUserData(snapU.exists() ? snapU.data() : {});
          const now = new Date();
          setOfferteData(snapO.docs.map(d => d.data()).filter(o => o.evergreen !== false || !o.scadenza || new Date(o.scadenza) >= now));
        } catch { setUserData({}); }
      }
      setLoading(false);
    });
  ...
  return <StudentPortal userData={userData} offerteGlobali={offerteData} />;
}
...
function AdminPanel({ adminUser }) {
  ...
  const loadAll = async () => {
    setLoadingData(true);
    try {
      const [snapS, snapL, snapO] = await Promise.all([
      ...
      setOfferteGlobali(snapO.docs.map(d => ({ id: d.id, ...d.data() })));
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI',sans-serif", color: C.text }}>
                  ...
                  <div key={m.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "20px 24px", marginBottom: 14 }}>
                    ...
                    {m.videolezioni?.map((v, vIdx) => (
                      <div key={vIdx} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "11px 16px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <input style={{ background: "none", border: "none", color: C.text, fontSize: 14, fontWeight: 600, width: "100%", outline: "none", fontFamily: "inherit" }} value={v.title} onChange={e => { const l = [...libreria]; l[mIdx].videolezioni[vIdx].title = e.target.value; setLibreria(l); }} />
                          <input style={{ background: "none", border: "none", color: C.muted, fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit" }} placeholder="URL iframe Bunny es. https://iframe.mediadelivery.net/embed/..." value={extractBunnyUrl(v.url)} onChange={e => { const l = [...libreria]; l[mIdx].videolezioni[vIdx].url = e.target.value; setLibreria(l); }} />
                        </div>
      ...
      {modalPromo && (
        <Modal onClose={() => setModalPromo(false)} title="🎁 Nuova offerta">
          ...
          <button style={{...btn(C.green), width: "100%", marginTop: 8}} onClick={async () => {
            const id = "offerta_" + Date.now();
            await setDoc(doc(db, "offerte", id), {...fPromo, id});
            await loadAll();
            setFPromo({ title: "", desc: "", price: "", badge: "", color: C.green, evergreen: true, scadenza: "" });
            setModalPromo(false);
            showToast("✅ Offerta aggiunta!");
...
// STUDENT PORTAL
// ═══════════════════════════════════════════════════════════════
function StudentPortal({ userData, offerteGlobali }) {
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:C.bg, fontFamily:"'Segoe UI',sans-serif", color:C.text }}>
                ...
                <div key={mIdx} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, marginBottom:14, overflow:"hidden" }}>
                  ...
                  {open&&m.videolezioni?.map((v,vIdx)=>(
                    <div key={vIdx} style={{ padding:"14px 24px", borderTop:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between", alignItems:"center", background:C.surface, cursor:"pointer" }} onClick={()=>setActiveVideo({...v,color:col})}>
                      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                        <div style={{ width:34, height:34, background:`${col}22`, border:`1px solid ${col}44`, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>{v.emoji||"🎬"}</div>
                        <div>
                          <div style={{ fontWeight:600, fontSize:14 }}>{v.title}</div>
                          <div style={{ fontSize:12, color:C.muted }}>{v.duration}</div>
                        </div>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      ...
      {showPromo&&(
        <Modal onClose={()=>setShowPromo(false)}>
          ...
          {(offerteGlobali?.length > 0 ? offerteGlobali : defaultPromos).map(o=>(
